...
class MtcnnDetector(object):
    ...
    def generate_bbox(self, cls_map, reg, scale, threshold):
        """
            generate bbox from feature cls_map according to the threshold 根据阈值从feature cls_map生成bbox
        Parameters:
        ----------
            cls_map: numpy array , n x m 
                detect score for each position 检测每个位置的得分
            reg: numpy array , n x m x 4
                bbox
            scale: float number
                scale of this detection 检测尺度
            threshold: float number
                detect threshold 检测阈值
        Returns:
        -------
            bbox array
        """
    ...
    def pad(self, bboxes, w, h):
        """
            pad the the bboxes, alse restrict the size of it 填充bboxes，并限制它的大小
        Parameters:
        ----------
    ...
    def detect_pnet(self, im):
        """Get face candidates through pnet 通过pnet获取候选面孔
        ...
        while min(current_height, current_width) > net_size:
            # return the result predicted by pnet 返回pnet预测的结果
            # cls_cls_map : H*w*2
            # reg: H*w*4
        ...
        # merge the detection from first stage 合并第一阶段的检测
    ...
    def detect_face(self, test_data):
        ...
        # test_data is iter_
        s_time = time.time()
        # 这里使用的是databatch in test_data,但是每次都得到的是None,应该是想拿到test_data 里面的图片，改为以下
        for databatch in test_data.imdb:
            ...
            # im = databatch
            im = cv2.imread(databatch)  # 应该用的是图片，需要加载图片
            if im is None:
                continue
            # pnet

            if self.pnet_detector:  # PNet 检测
                st = time.time()
                # ignore landmark
                boxes, boxes_c, landmark = self.detect_pnet(im)  # landmark 暂时是没有作用的
