...
def read_single_tfrecord(tfrecord_file, batch_size, net):
    ...
    image_features = tf.parse_single_example(
        ...
        features={
            ...
            'image/roi': tf.FixedLenFeature([4], tf.float32),  # TODO:
    ...
    if net == 'PNet':
        ...
        image_size = 48
    image = tf.decode_raw(image_features['image/encoded'], tf.uint8)  # 将字符串的字节重新解释为数字向量
    ...
    image, label, roi = tf.train.batch(  # 在张量中创建张量的批次
        # [image, label, roi],
        [image, label, roi],
