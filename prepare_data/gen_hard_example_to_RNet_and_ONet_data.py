...
def t_net(prefix, epoch, batch_size, test_mode="PNet", thresh=[0.6, 0.6, 0.7],
    ...
    """
    :param prefix:模型的路径
    :param epoch:当一个完整的数据集通过了神经网络一次并且返回了一次，这个过程称为一个 epoch
    :param batch_size:batchsize 的正确选择是为了在内存效率和内存容量之间寻找最佳平衡
    :param test_mode:PNet、RNet、ONet
    :param thresh:阈值
    :param min_face_size:
    :param stride:步长
    :param slide_window:
    ...
    print(model_path[0])
    # load pnet model
    if slide_window:  # TODO:加载PNet网络模型参数
        PNet = Detector(P_Net, 12, batch_size[0], model_path[0])  # 初始化 加载模型
    else:
        PNet = FcnDetector(P_Net, model_path[0])  # 初始化 加载模型
    detectors[0] = PNet

    # load rnet model
    if test_mode in ["RNet", "ONet"]:
        print("==================================", test_mode)
        RNet = Detector(R_Net, 24, batch_size[1], model_path[1])  # 初始化 加载模型
        detectors[1] = RNet

    # load onet model
    if test_mode == "ONet":
        print("==================================", test_mode)
        ONet = Detector(O_Net, 48, batch_size[2], model_path[2])  # 初始化 加载模型
        detectors[2] = ONet

    basedir = '../data/'
    # anno_file
    # filename = './wider_face_train_bbx_gt.txt'
    filepath = "../data/label_train.txt"
    # read anotation(type:dict), include 'images' and 'bboxes'
    # data = read_annotation(basedir, filename)
    data = read_anno(filepath)
    # 定义MtcnnDetector对象，初始化mtcnn_detector
...
def parse_args():
    ...
    parser.add_argument('--prefix', dest='prefix', help='prefix of model name', nargs="+",
                        default=['../data/MTCNN_model/PNet/PNet',
                                 '../data/MTCNN_model/RNet/RNet',
